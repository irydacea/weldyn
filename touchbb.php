...
if ($get == 'replypm' || $get == 'postpm') {
  ...
  if ($user->data['is_registered']) {
    if ($config['allow_privmsg']) {
      ...
      $message_parser->parse($enable_bbcode, ($config['allow_post_links']) ? $enable_urls : false, $enable_smilies, true, true, true, $config['allow_post_links']);
...
if ($get == 'myposts') {
  ...
  if ($user->data['is_registered']) {
    ...
    $sql = "SELECT t.topic_id,t.topic_moved_id,t.topic_title,t.topic_last_post_time,t.topic_last_poster_name,topic_first_poster_name,topic_time,topic_views,$replyStr,t.forum_id" . (($user->data['is_registered']&&$config['load_db_lastread'])?',tt.mark_time':'') . " FROM (" . POSTS_TABLE . " p, " . TOPICS_TABLE . " t," . FORUMS_TABLE . " f)" . (($user->data['is_registered']&&$config['load_db_lastread'])?' LEFT JOIN ' . TOPICS_TRACK_TABLE . ' tt ON tt.user_id = ' . $db->sql_escape($user->data['user_id']) . ' AND t.topic_id = tt.topic_id':'') . " WHERE (p.forum_id='" . implode("' OR p.forum_id='", $forumIDs) . "') AND p.topic_id = t.topic_id" . $sql_approved . " AND topic_poster=" . $db->sql_escape($user->data['user_id']) . " AND t.forum_id=f.forum_id AND forum_password='' GROUP BY topic_id ORDER BY t.topic_last_post_time DESC LIMIT " . $db->sql_escape($page) . ',' . $db->sql_escape($msgsPerPage);
    $result = $db->sql_query($sql);
    while ($row = $db->sql_fetchrow($result)) {
      $data[] = $row;
    }
...
if ($get == 'sent') {
  ...
  if ($user->data['is_registered']) {
    if ($config['allow_privmsg']) {
      ...
      $sql = 'SELECT t.*, p.root_level, p.message_time, p.message_subject, p.icon_id, p.to_address, p.message_attachment, p.bcc_address, u.username, u.username_clean, u.user_colour FROM ' . PRIVMSGS_TO_TABLE . ' t, ' . PRIVMSGS_TABLE . ' p, ' . USERS_TABLE . " u WHERE t.user_id = " . $db->sql_escape($user->data['user_id']) . " AND p.author_id = u.user_id AND (t.folder_id=" . PRIVMSGS_OUTBOX . " OR t.folder_id=" . PRIVMSGS_SENTBOX . ") AND t.msg_id = p.msg_id ORDER BY folder_id,p.message_time DESC LIMIT " . $db->sql_escape($page) . ',' . $db->sql_escape($msgsPerPage);
      $result = $db->sql_query($sql);
...
if ($get == 'reply' || $get == 'post') {
  ...
  if ($user->data['is_registered']) {
    ...
    if (!$title && $topic_id) {
      $sql = "SELECT topic_title FROM " . TOPICS_TABLE . " WHERE topic_id=" . $db->sql_escape($topic_id);
      $result = $db->sql_query($sql);
    ...
    if ($update_message) {
      ...
      $message_parser->parse($post_data['enable_bbcode'], ($config['allow_post_links']) ? $post_data['enable_urls'] : false, $post_data['enable_smilies'], true, true, true, $config['allow_post_links']);
...
if ($get == 'topic') {
  ...
  if (@version_compare($appVersion, '1.4', '>=')) {
    $sql .= ' LIMIT ' . $db->sql_escape($page) . ',' . $db->sql_escape($msgsPerPage);
  }
  ...
  $sql = $db->sql_build_query('SELECT', array(
    ...
    'LEFT_JOIN' => array(
      array(
        ...
        'ON' => 'z.user_id = ' . $db->sql_escape($user->data['user_id']) . ' AND z.zebra_id = p.poster_id'
      )
    ),
...
if ($get == 'forum') {
  ...
  if ($config['load_db_lastread'] && $user->data['is_registered']) {
    $sql_from .= ' LEFT JOIN ' . FORUMS_TRACK_TABLE . ' ft ON (ft.user_id = ' . $db->sql_escape($user->data['user_id']) . ' AND ft.forum_id = f.forum_id)';
    $lastread_select .= ', ft.mark_time';
  }

  if ($user->data['is_registered']) {
    $sql_from .= ' LEFT JOIN ' . FORUMS_WATCH_TABLE . ' fw ON (fw.forum_id = f.forum_id AND fw.user_id = ' . $db->sql_escape($user->data['user_id']) . ')';
  ...
  if ($showTopics) {
    ...
    if ($user->data['is_registered']) {
      if ($config['load_db_track']) {
        $sql_array['LEFT_JOIN'][] = array('FROM' => array(TOPICS_POSTED_TABLE => 'tp'), 'ON' => 'tp.topic_id = t.topic_id AND tp.user_id = ' . $db->sql_escape($user->data['user_id']));
        $sql_array['SELECT'] .= ', tp.topic_posted';
      }

      if ($config['load_db_lastread']) {
        $sql_array['LEFT_JOIN'][] = array('FROM' => array(TOPICS_TRACK_TABLE => 'tt'), 'ON' => 'tt.topic_id = t.topic_id AND tt.user_id = ' . $db->sql_escape($user->data['user_id']));
        $sql_array['SELECT'] .= ', tt.mark_time';

        if ($s_display_active && sizeof($active_forum_ary)) {
          $sql_array['LEFT_JOIN'][] = array('FROM' => array(FORUMS_TRACK_TABLE => 'ft'), 'ON' => 'ft.forum_id = t.forum_id AND ft.user_id = ' . $db->sql_escape($user->data['user_id']));
    ...
    $sql = 'SELECT t.topic_id FROM ' . TOPICS_TABLE . " t WHERE $sql_where AND t.topic_type IN (" . POST_NORMAL . ', ' . POST_STICKY . ") $sql_approved ORDER BY t.topic_type DESC,t.topic_last_post_time DESC LIMIT " . $db->sql_escape($page) . ',' . $db->sql_escape($msgsPerPage);
    $result = $db->sql_query($sql);
